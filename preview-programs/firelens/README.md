...
Fluentd and Fluent Bit both support filtering of logs based on their content. FireLens provides a simple short hand for enabling this. The options section of a container's logConfiguration can contain the special keys `exclude-pattern` and `include-pattern` that take regular expressions as their values. The `exclude-pattern` key will cause all logs that match its regular expression to be dropped. With `include-pattern` key, only logs which match its regular expression will be sent. These keys can be used together.
...
Your Fluent Bit image must be configured to use a configuration file at `/fluent-bit/etc/fluent-bit.conf`. The `aws-for-fluent-bit` image and the `fluent/fluent-bit` images all use this default path. The command in the Dockerfile for your Fluent Bit image should look something like the following:

```
CMD ["/fluent-bit/bin/fluent-bit", "-c", "/fluent-bit/etc/fluent-bit.conf"]
```

Your Fluentd image must be configured to use a configuration file at `/fluentd/etc/fluent.conf`. The `fluent/fluentd` images all use this default path. See the official [fluentd-docker-image](https://github.com/fluent/fluentd-docker-image) repo for examples.
